...
//   FaExternalLinkAlt,
// } from "react-icons/fa";
// import { apiFetch, authAPI, testsAPI } from "../services/api";
// import { useAuth } from "../context/AuthContext";
// import { socket } from "../services/socket";
...
//   const viaToken = Boolean(location.state?.viaToken);
//   // Token passed from TokenTestPage so launchTest can call startByToken
...
//       // ── NEW: Record that this logged-in student viewed the test ────────────
//       // Server deduplicates — safe to call every render.
//       if (user?._id) {
//         authAPI.recordTestView(testData._id).catch(() => {});
//       }
...
//     // ── NEW: signal that the student clicked "Start" via a token link ───────
//     // This lets the backend mark the visit as "started" for conversion tracking.
//     if (accessToken) {
//       testsAPI.startByToken(accessToken).catch(() => {});
//     }
...
//   // Owner clicks leaderboard row → navigate to full result view
//   const handleViewStudentResult = async (leaderboardEntry) => {
//     const resultId = leaderboardEntry._id;
//     if (!resultId) return;
//     setLoadingResultId(resultId);
...
//   const handleDelete = async () => {
//     try {
//       await testsAPI.delete(test._id);
//       toast({ title: "Test deleted", status: "success" });
//       navigate(-1);
//     } catch (e) {
//       toast({ title: e.message, status: "error" });
...
//     try {
//       // 1. Full result for this leaderboard entry (first attempt)
//       const res = await resultsAPI.getById(resultId);
//       const r = res.data || res;
//       const studentName = r.studentId?.Name || r.studentId?.Email || "Student";
//       const studentId = String(r.studentId?._id || r.studentId);
...
//       let allAttempts = [];
//       try {
//         const allRes = await resultsAPI.getForTest(test._id);
//         const allRaw = allRes.data || [];
//         // keep only this student's attempts, sorted oldest first
//         allAttempts = allRaw
//           .filter((a) => String(a.studentId?._id || a.studentId) === studentId)
...
import { apiFetch, authAPI, testsAPI, resultsAPI } from "../services/api";
import { useAuth } from "../context/AuthContext";
import { socket } from "../services/socket";
...
export default function TestDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (isObjectId) {
        ...
        testData = res.data ?? res;
      } else {
        try {
          const res = await testsAPI.getBySlug(id);
          testData = res.data ?? res;
        } catch (e) {
          if (user?._id) {
            const res = await testsAPI.getById(id);
      ...
      const testId = testData._id;
      const [statsRes, lbRes] = await Promise.all([
        testsAPI.getStats(testId).catch(() => ({ data: null })),
        testsAPI.getLeaderboard(testId).catch(() => ({ data: [] })),
      ]);
      setStats(statsRes.data);
      setLeaderboard(firstAttemptPerStudent(lbRes.data || []));
      if (user?._id) {
        resultsAPI.getMyResults({ testId })
          .then((r) => setMyResult((r.data ?? r)?.[0] || null))
          .catch(() => {});
      }
    } catch (e) {
      toast({ title: e.message, status: "error" });
  ...
  useEffect(() => {
    ...
    const onAttempted = ({ testId }) => {
      if (String(testId) === String(test._id)) {
        Promise.all([
          testsAPI.getStats(test._id).catch(() => ({ data: null })),
          testsAPI.getLeaderboard(test._id).catch(() => ({
            data: [],
          })),
        ]).then(([sRes, lbRes]) => {
          setStats(sRes.data);
