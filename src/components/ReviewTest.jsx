...
import React, { useState } from "react";
import {
  Box, Flex, Text, Icon, VStack, Button, Grid, Center,
...
/* ── status config ────────────────────────────────────────────── */
const STATUS_MAP = {
  correct:        { label: "Correct",          color: "#15803d", bg: "#f0fdf4", border: "#86efac", cell: "#16a34a", icon: FaCheckCircle },
  incorrect:      { label: "Incorrect",         color: "#b91c1c", bg: "#fef2f2", border: "#fca5a5", cell: "#ef4444", icon: FaTimesCircle },
  markedAnswered: { label: "Marked & Answered", color: "#6d28d9", bg: "#f5f3ff", border: "#c4b5fd", cell: "#7c3aed", icon: FaFlag       },
  markedSkipped:  { label: "Marked (Skipped)",  color: "#b45309", bg: "#fffbeb", border: "#fcd34d", cell: "#d97706", icon: FaFlag       },
  skipped:        { label: "Not Attempted",     color: "#475569", bg: "#f8fafc", border: "#cbd5e1", cell: "#94a3b8", icon: FaCircle     },
  answered:       { label: "Answered",          color: "#1d4ed8", bg: "#eff6ff", border: "#93c5fd", cell: "#2563eb", icon: FaCheckCircle },
  notVisited:     { label: "Not Visited",       color: "#94a3b8", bg: "#f9fafb", border: "#e5e7eb", cell: "#d1d5db", icon: FaCircle     },
};
      ...
      <Box px={4} pt={5} pb={4} borderBottom="1px solid #f1f5f9" flexShrink={0}>
        ...
        <Grid templateColumns="1fr 1fr" gap={2}>
          {LEGEND.map(([key, label]) => (
            <Flex key={key} align="center" gap={2}>
              <Box w="10px" h="10px" borderRadius="3px" flexShrink={0}
                bg={STATUS_MAP[key].cell}
                border={key === "skipped" || key === "notVisited"
                  ? "1.5px solid #cbd5e1" : "none"} />
              <Text fontSize="11px" color="#475569" fontWeight={500} noOfLines={1}>
                {label}
              </Text>
            </Flex>
          ))}
      ...
      <Box px={4} py={4} flex={1} overflowY="auto"
        ...
        <Grid templateColumns="repeat(5, 1fr)" gap={2}>
          {questions.map((_, i) => {
            ...
            return (
              <Center
                key={i} h="34px" borderRadius="6px" cursor="pointer"
                ...
                onClick={() => { setCurrent(i); if (onClose) onClose(); }}
              >
                {i + 1}
              </Center>
            );
...
function QuestionCard({ question, index, allAnswers, sa, questionTimes }) {
  ...
  const cid = typeof question.answer === "number"
    ? question.answer
    : question.answer - 1;
  ...
  return (
    <Box bg="white" borderRadius="12px" border="1px solid #e5e7eb"
      ...
      <Flex align="center" justify="space-between"
        ...
        <Flex align="center" gap={2.5}>
          <Center w="28px" h="28px" borderRadius="6px"
            bg={cfg.bg} border={`1px solid ${cfg.border}`} flexShrink={0}>
            <Text fontSize="12px" fontWeight={900} color={cfg.color}>
              {index + 1}
            </Text>
          </Center>
          <Flex align="center" gap={1.5} px={2.5} py="4px"
            borderRadius="20px" bg={cfg.bg} border={`1px solid ${cfg.border}`}>
        ...
        {timeStr && (
          <Flex align="center" gap={1.5} px={2.5} py="4px"
            borderRadius="20px" bg="#f9fafb" border="1px solid #e5e7eb">
      ...
      <Box px={{ base: 4, md: 5 }} pt={4} pb={5}>
        ...
        <Text fontSize={{ base: "15px", md: "16px" }} fontWeight={600}
          color="#0f172a" lineHeight="1.8" mb={4}>
          {question.qus}
        </Text>
          ...
          {question.options?.map((opt, i) => {
            ...
            return (
              <Flex key={i} align="center" gap={3}
                px={4} py={3} borderRadius="8px"
                border="1.5px solid"
                borderColor={isC ? "#86efac" : isW ? "#fca5a5" : "#e5e7eb"}
                bg={isC ? "#f0fdf4" : isW ? "#fef2f2" : "#fafafa"}>

                <Center w="28px" h="28px" borderRadius="6px" flexShrink={0}
                ...
                <Text flex={1} fontSize="14px"
                  ...
                  fontWeight={isC || isU ? 600 : 400} lineHeight="1.5">
                  {opt}
                </Text>

                <Flex gap={1.5} flexShrink={0}>
                  {isC && (
                    <Box px={2} py="2px" borderRadius="4px"
        ...
        {/* explanation */}
        {question.explanation && (
          <Box p={4} bg="#eff6ff" borderRadius="8px"
  ...
  if (!questions.length) {
    return (
      <Flex minH="100vh" align="center" justify="center"
        direction="column" gap={3} bg="#f9fafb"
        fontFamily="'DM Sans',sans-serif">
        <Icon as={FaChartBar} fontSize="44px" color="#d1d5db" />
        <Text fontSize="17px" fontWeight={700} color="#111827">
          No review data
        </Text>
        <Button onClick={() => navigate(-1)} bg="#111827" color="white"
          borderRadius="8px" fontWeight={700} h="42px" px={5} mt={1}
          _hover={{ bg: "#1f2937" }}>
          Go Back
        </Button>
      </Flex>
  ...
  return (
    <Box bg="#f3f4f6" minH="100vh" fontFamily="'DM Sans',sans-serif">
      ...
      <Box
        ...
        <Flex
          ...
          <Flex align="center" gap={3} flex={1} minW={0}>
            <Center
              w="32px" h="32px" borderRadius="8px" flexShrink={0}
              bg="rgba(255,255,255,.08)" border="1px solid rgba(255,255,255,.12)"
              cursor="pointer" onClick={() => navigate(-1)}
              _hover={{ bg: "rgba(255,255,255,.15)" }} transition="background .15s">
              <Icon as={FaArrowLeft} color="rgba(255,255,255,.8)" fontSize="12px" />
            </Center>
            <Box minW={0}>
              <Text fontSize={{ base: "13px", md: "15px" }} fontWeight={800}
                color="white" noOfLines={1} letterSpacing="-0.2px">
                {testTitle}
              </Text>
              <Text fontSize="10px" color="rgba(255,255,255,.4)"
                fontWeight={600} textTransform="uppercase" letterSpacing=".8px">
                Review Mode
              </Text>
            </Box>
          </Flex>
          ...
          {/* center: counter */}
          <Box px={4} py="6px" borderRadius="8px" flexShrink={0}
            bg="rgba(255,255,255,.08)" border="1px solid rgba(255,255,255,.12)">
          ...
          {!isLg && (
            <Center
              w="32px" h="32px" borderRadius="8px" flexShrink={0} ml={3}
              bg="rgba(255,255,255,.08)" border="1px solid rgba(255,255,255,.12)"
      ...
      <Flex>
        ...
        {isLg && (
          <Box
            w="240px" flexShrink={0} bg="white"
            borderRight="1px solid #e5e7eb"
            position="sticky" top={HDR.md || "60px"}
            h={`calc(100vh - 60px)`}
            overflowY="auto"
            css={{
              "&::-webkit-scrollbar": { width: "3px" },
              "&::-webkit-scrollbar-thumb": { background: "#e5e7eb", borderRadius: "4px" },
            }}>
            <Sidebar
              questions={questions} current={cur}
              setCurrent={setCur} sa={sa} />
          </Box>
        )}
          ...
          <Drawer isOpen={isOpen} onClose={onClose} placement="left" size="xs">
            ...
            <DrawerContent bg="white" maxW="260px">
              ...
              <DrawerBody p={0} pt={10}>
                <Sidebar
                  questions={questions} current={cur}
                  setCurrent={setCur} sa={sa} onClose={onClose} />
              </DrawerBody>
            </DrawerContent>
          </Drawer>
        )}
        ...
        <Box flex={1} minW={0}>
          ...
          {/* sticky sub-header */}
          <Flex
            px={{ base: 4, md: 6, lg: 8 }} py={3}
            bg="white" borderBottom="1px solid #e5e7eb"
            align="center" gap={3}
            position="sticky" top={HDR} zIndex={40}>
            <Box px={3} py="5px" borderRadius="6px" bg="#0f172a">
              <Text fontSize="11px" fontWeight={800} color="white" letterSpacing=".3px">
                Q {cur + 1} / {questions.length}
              </Text>
            </Box>
            <Flex align="center" gap={1.5} px={2.5} py="4px"
              borderRadius="20px" bg={curCfg.bg} border={`1px solid ${curCfg.border}`}>
          ...
          <Box px={{ base: 3, md: 5, lg: 8 }} pb={8}>
            <Flex align="center" justify="space-between" gap={3}
              bg="white" borderRadius="10px" border="1px solid #e5e7eb"
              px={3} py={2.5} boxShadow="0 1px 6px rgba(0,0,0,.04)">

              <Button
                leftIcon={<Icon as={FaArrowLeft} fontSize="10px" />}
                isDisabled={cur === 0}
                onClick={() => setCur(c => Math.max(0, c - 1))}
                bg="#f3f4f6" color="#374151" borderRadius="8px"
                fontWeight={700} fontSize="13px" h="38px" px={4}
                border="1px solid #e5e7eb"
                _hover={{ bg: "#e9eaec" }}
                _disabled={{ opacity: .35 }}
                transition="all .12s">
                Prev
              </Button>
              ...
              <Button
                ...
                bg="#0f172a" color="white" borderRadius="8px"
                fontWeight={700} fontSize="13px" h="38px" px={4}
                _hover={{ bg: "#1e293b", boxShadow: "0 3px 10px rgba(0,0,0,.2)" }}
                _disabled={{ opacity: .35 }}
                transition="all .12s">
                Next
              </Button>
